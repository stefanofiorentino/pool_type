...
TEST_F(FakeHandleFixture, on_publish) {
    auto called{false};
    fakeHandle->on<FakeEvent1>([&called](auto&& event, auto &handler) {
        called = true;
    });
...
TEST_F(FakeHandleFixture, on_empty) {
    ...
    fakeHandle->on<FakeEvent1>([](auto&& event, auto &handler) {
    });
...
TEST_F(FakeHandleFixture, on_clear) {
    ASSERT_TRUE(fakeHandle->empty());
    fakeHandle->on<FakeEvent1>([](auto&& event, auto &handler) { FAIL(); });
    ASSERT_FALSE(fakeHandle->empty());
    fakeHandle->clear();
    ASSERT_TRUE(fakeHandle->empty());
}
